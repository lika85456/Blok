...
public class SquareGroup extends ViewGroup {
    ...
    public void fromPiece(Piece piece) {
        ...
        PointF mass = piece.getMass();
        int marginLeft = (int) ((mass.x - 2f) * squareSize);
        int marginTop = (int) ((mass.y - 2f) * squareSize);
        for (int i = 0; i < piece.list.size(); i++) {
            ...
            temp.layout(temp.x * squareSize - marginLeft, temp.y * squareSize - marginTop, (temp.x + 1) * squareSize - marginLeft, (temp.y + 1) * squareSize - marginTop);
        }
        this.requestLayout();
    }
